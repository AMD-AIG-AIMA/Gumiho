...
from vllm.spec_decode.gumiho_worker import GumihoWorker
    ...
    * The scoring forward pass is done without an MQA kernel, which is
    ...
    def create_worker(
        ...
        if ngram_prompt_lookup_max > 0:
            ...
            elif draft_model_config.hf_config.model_type == "gumiho":
                if draft_tp == 1:
                    if current_platform.is_cuda_alike():
                        draft_worker_kwargs[
                            "model_runner_cls"] = TP1DraftModelRunner
                proposer_worker = GumihoWorker(**draft_worker_kwargs)
            else:
                if draft_tp == 1:
                    if current_platform.is_cuda_alike():
                        draft_worker_kwargs[
