...
class Timer:
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        logger.info(f'{self.name} took {elapsed*1000:.3f} ms')
...
def initialize_tree0(input_ids, model, past_key_values, logits_processor):
    ...
    #
    #     draft_tokens, retrieve_indices,tree_mask,tree_position_ids = self.gumiho_layer.topK_genrate(hidden_states, input_ids, self.base_model.lm_head)
    #     if output_orig:
...
def initialize_tree(input_ids, model, past_key_values, logits_processor):
    ...
    draft_tokens, retrieve_indices,tree_mask,tree_position_ids = model.gumiho_layer.topK_genrate(hidden_states, input_ids, model.base_model.lm_head,logits_processor)
...
def update_inference_inputs(
    ...
    draft_tokens, retrieve_indices,tree_mask,tree_position_ids = model.gumiho_layer.topK_genrate(accept_hidden_state_new,
                                              input_ids=torch.cat((input_ids, token.to(input_ids.device)), dim=1),
