...
class ParallelMLPs(nn.Module):
    def __init__(self, hidden_size, mlp_num):
        super().__init__()
        self.mlp = nn.ModuleList([
                        nn.Sequential(
                            noResBlock(hidden_size * 2, hidden_size),
...
class Model(nn.Module):
    def __init__(self, config, load_emb=False, path=None, bias=True, total_tokens=63, depth=5, top_k=10, threshold=1.0, args=None):
        ...
        self.NumLayer_decoder = config.num_hidden_layers
        self.layers = nn.ModuleList([LlamaDecoderLayer(config, index) for index in range(self.NumLayer_decoder)])
        self.serial_head_num = args.serial_head_num
    
        
        self.mlp_num = self.args.mlp_num
        self.mlp = ParallelMLPs(config.hidden_size, self.mlp_num)
    ...
    def init_tree(self):
        ...
        self.position_ids = torch.zeros(self.args.top_k, device=self.embed_tokens.weight.device, dtype=torch.long)
        self.tree_mask_init = self.tree_mask_init.to(self.embed_tokens.weight.device)

    def compile_mlp(self):
        device = next(self.mlp.parameters()).device
        self.mlp.eval()
        self.mlp.to(torch.float16)
        dummy_input = torch.randn(self.args.top_k, 1, 2*self.config.hidden_size, device=device, dtype=torch.float16)
        compiled_model = torch.compile(self.mlp, mode="max-autotune")

        with torch.inference_mode():
            for _ in range(50):
                _ = compiled_model(dummy_input)
            torch.cuda.synchronize()
        self.mlp = compiled_model
    ...
    def forward(
            ...
            std=None,
            mode="train"  # "train" "mlp_generate" "gumiho_generate"
    ):
        ...
        if mode == "mlp_generate":
            inputs_embeds = inputs_embeds.to(hidden_states.dtype)
            hidden_states = self.fc(torch.cat((inputs_embeds, hidden_states), dim=-1))
            mlp_inputs = hidden_states.view(batch_size, 1, -1) 
            
            ret_hidden_states = self.mlp(mlp_inputs) 
    ...
    def topK_genrate(self, hidden_states, input_ids, head, logits_processor):
        ...
        for i in range(TwoLayerDecoderNum):
            ...
            _, max_index = torch.max(scores.view(-1), dim=0)
            top1_score_index.append(max_index+previous_score_num)
            previous_score_num += scores.numel()
        ...
        for mlp_i in range(mlp_gen_token_num):
            ...
            top = torch.topk(last_p, mlp_topk, dim=-1)  # [10,5], mlp_topk=5
            token_index, token_p = top.indices, top.values
                
            cu_scores = token_p + scores[:, None]  # [10,5]+[10,1]=[10,5]
            cu_scores = cu_scores.view(-1)  # [100] [1w] [100w]

            if pruning > 0:  
                mlp_top = torch.topk(cu_scores, pruning) # [50]->[7]
